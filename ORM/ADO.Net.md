...
**ADO.net has two main comonents that are used for accessing and manipulatind data.**
...
- Connection string contains the information about the database server, server type, databse name, cnnection type, and database user credentials.
...
1.  **ExecuteReader()**
    this is used when we need to select, read data from database.
...
3. **IsClosed-** it rtrieve the boolean value that indicates whether the specified System.Data.SqlClient.SqlDataReader instance has been closed or not

4. **Item[String]-** it get the value of the specified column in its native formate given the column name.

5. **Item[int32]-** it gets the value of the specified column in its native formate given the column ordinal
